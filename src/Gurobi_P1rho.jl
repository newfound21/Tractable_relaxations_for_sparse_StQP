...
#
# This code solves the MIQP formulation (P1) in the manuscript.
#
# Inputs:
# 
...
function Gurobi_P1rho(Q0, k, timelimit)
    ...
    @variable(model_sparse_QP, u[1:n], Bin)

    @objective(model_sparse_QP, Min,  dot(Q0*x, x))

    @constraint(model_sparse_QP, sum(x) == 1)
    
    @constraint(model_sparse_QP, sum(u) == k)
    
    @constraint(model_sparse_QP, x .<= u)
