...
#
# This code generates an SPN instance such that x_hat is the unique optimal 
...
function Generate_Q_SPN(seed_n, x_hat)
    ...
    # Generate N with uniformly distributed entries in [0,3]

    N[1:nonzero_num,nonzero_num+1:n] = rand(Uniform(0,3),nonzero_num,n - nonzero_num)

    N[nonzero_num+1:n,1:nonzero_num] = N[1:nonzero_num,nonzero_num+1:n]'

    N[nonzero_num+1:n,nonzero_num+1:n] = Symmetric(rand(Uniform(0,3),n - nonzero_num,n - nonzero_num))

     # Q_1 before reordering

    Q_1 = (diagm(ones(n)) - ones(n) * x_hat[perm]') * R * (diagm(ones(n)) - ones(n) * x_hat[perm]')' + N
