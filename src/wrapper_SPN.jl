...
# on SPN instances. Seed is initialised from 0 but only results from 
...
for n in nset
    ...
    for rho0ratio in rho0ratioset
	    ...
	    for seed in seedset
            ...
            Q_1 = Generate_Q_SPN(seed, x_hat)

            Lambda, U = eigen(Q_1)

            mineig = minimum(Lambda)
            ...
            for rhoratio in rhoratioset
                ...
                current_vec = [n;rho_0;seed;rho; mineig; current_outputs]
    ...
    CSV.write("./n"*string(n)*"-SPN.csv", df_for_each_rho)
